...
#
# 用法:
#   bash <(curl -fsSL $AGENT_HUB_URL/api/v1/dist/install.sh) [HUB_URL]
#   例: bash <(curl -fsSL http://43.163.76.175:20100/api/v1/dist/install.sh)
#
# 流程: 拉取 SDK → 初始化钱包身份(agent.json) → 指引注册/启动聊天微服务
# =============================================================================
set -euo pipefail
...
test -f "$WORK_DIR/agent_sdk/__init__.py" && test -f "$WORK_DIR/agent_cli.py" \
  && echo "    ✅ SDK 就绪（$WORK_DIR）" \
  || { echo "❌ SDK 解压校验失败"; exit 1; }

echo "==> 初始化身份（生成钱包，持久化 ~/.agent-marketplace/agent.json）..."
python3 "$WORK_DIR/agent_cli.py" --hub "$HUB_URL" init \
  || { echo "❌ 身份初始化失败"; exit 1; }
...
echo "      --domain finance --subdomain quantitative_trading \\"
echo "      --skills backtesting --price 0.005 --auto-price"
