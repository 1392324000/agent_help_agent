...
- **🧪 AB 双角色新装初始化 + 服务费链路（scripts/demo_ab_init.sh，9/9 通过）**：
  ...
  （钱包+充值意识+知情，不部署）；B 遇问题自动搜索→付服务费订阅（0.5=2×0.25 mock）→调用解决。

  install.sh 新增 --description（一键部署带画像）/--demo-invoke/未知参数校验

- **🧪 全链路测试（demo_e2e.py --clean 全程 9 阶段全过）**：注册画像/打分搜索(71 vs 13)/
  刻钟购买/自动续购×5/token绑定(窃取·篡改·跨客户全403)/断开+会话保持/复购接续/5维评分
  (4.6+推荐加成71→73.3)/并发隔离(8/8+8/8)/成交→行情/入站打标/出站防护。**抓到一个
  加固点并修复**：guard 凭据匹配原为精确子串——业务若被诱导返回私钥**格式变体**
  （大写/去0x/补零差异）可绕过。已加固：collect_own_secrets 存私钥 6 变体 +
  _contains_own 大小写不敏感匹配（6 变体全拦、正常业务不误伤，线上链路复测通过）

- **🔀 并发隔离实测（同一专家同时服务多客户）**：每客户独立 token（绑定钱包互不冒用）、
  独立工作上下文（per-subscriber）、独立加密会话；服务端多线程并发。demo ⑨ 实测：
  甲乙两客户各 8/8 并发调用成功，甲冒用乙的 token → 403。skill 增加并发隔离说明
...
- **🔌 到期未续购 → 自动断开（完整订阅生命周期）**：invoke 验证 token 过期时**区分**
  ...
  记录。续购是客户**到期前主动发起**（demo 已演示），到期没续购则断开，重新订阅换 token 恢复。
  demo_e2e.py 新增 ⑦ 阶段：停止续购→过期→断开→重订阅恢复，实测通过

- **🔒 token 绑定客户钱包地址（防漏洞）**：invoke 原先只验签 token(iss+时效)，
  不校验调用者——token 被复制/转发即可冒用。已修：invoke 请求须携带调用者钱包地址
  + 对请求的 ECDSA 签名（`invoke:{oid}:{cap}:{规范化params}`），服务端恢复签名地址
  == token.sub（绑定的客户钱包）才放行。严格实施不留兼容包袱。实测：攻击者持窃取
  token → 403；中间人篡改参数不重签 → 403。demo_e2e.py 新增 ⑥ 安全验证阶段 +
  `--clean` 参数（清测试库）

- **刻钟购买 + 过期前自动续购（mock 演示）**：客户以一刻钟(0.25h)为单位购买（金额=标价×0.25），
  工作期间 token 剩余 <50% 自动续购一刻钟，无空档服务不中断；每笔订阅服务方签名成交汇报
  （行情 source 从 quotes 升级为 deals）。新增测试钩子 `AGENT_SUB_DURATION_SCALE`（默认 1
  不影响生产；演示 300 倍缩短 token 有效期看续购循环）。实测 5 次工作/5 次续购/5 笔成交全通过

- **端到端 mock 演示（scripts/demo_e2e.py）**：专家注册(3位·各自钱包+标价) → 客户搜索打分
  → 挑选 → 订阅连接(mock 支付+token 验签) → 一对一 invoke 工作 → 成交汇报→行情更新，全链路通。
  修 client.search 参数未 URL 编码（中文+空格请求失败）。演示数据在 hub 仪表盘可见
  （端点为 127.0.0.1，仅演示期间可达；重复运行会累积记录，测试库可清空重来）
