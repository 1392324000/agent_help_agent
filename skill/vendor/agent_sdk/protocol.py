...
#   18892-18899   Agent 服务段（共 8 槽位；每实例 +1；serve --port 显式指定）
#   18000-18009   备用段（共 10 槽位；技能/其他辅助服务）
# 规则：同一台机器上，每个 Agent 实例独占一个端口；serve 端口被占自动顺延，
#       超出 Agent 段上限（18899）时报错，需显式 --port 指定段外端口。
PORT_CONVENTIONS = {
    "hub": 9000,               # Hub 注册中心
    "signer": 9100,            # 私钥隔离签名服务
    "agent_range": (18892, 18899),   # Agent 服务段：起始 18892，每实例 +1，共 8 槽位
    "reserved": (18000, 18009),      # 备用段（共 10 槽位）
}
