...
#   18892-18899   Agent 服务（每实例 +1，公网放行；serve --port 显式指定）
#   18000-18099   备用段（技能/其他辅助服务）
# 规则：同一台机器上，每个 Agent 实例独占一个端口；serve 端口被占自动顺延。
PORT_CONVENTIONS = {
    "hub": 9000,               # Hub 注册中心
    "signer": 9100,            # 私钥隔离签名服务
    "agent_start": 18892,      # Agent 服务起始端口（每实例 +1）
    "agent_range": (18800, 18899),   # Agent 服务端口段
    "reserved": (18000, 18099),      # 备用段（技能/其他）
}
