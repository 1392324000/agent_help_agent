...
def _post(url: str, payload: dict, timeout: float = 15) -> dict:
    ...
    try:
        ...
        raise HubError(f"cannot connect to {url}: {e.reason}") from e
...
class HubClient:
    ...
    def refresh(self, endpoint: str) -> dict:
        ...
        if not self.agent_token:
            raise HubError("no agent_token (not registered or credentials not loaded)")
    ...
    def search(self, domain: str | None = None, subdomain: str | None = None,
        ...
        if not resp.get("ok"):
            raise HubError(resp.get("error", "search failed"))
        return resp["agents"]
    ...
    def get_agent(self, agent_id: str) -> dict:
        ...
        if not resp.get("ok"):
            raise HubError(resp.get("error", "agent not found"))
    ...
    def get_peer_manifest(self, peer_agent_id: str) -> dict:
        ...
        if not resp.get("ok"):
            raise HubError(resp.get("error", "/manifest fetch failed"))
    ...
    def subscribe_to_peer(self, peer_agent_id: str, duration_hours: float,
        ...
        if not tx_hash:
            if sub.get("chain") == "mock":
                ...
                from .chain import load_chain, get_balances, transfer_erc20, TX_GAS_BUDGET_BNB
                cfg = load_chain("bsc")
                bal = get_balances(cfg, self.wallet.address)
                ...
                if bal["native"] < TX_GAS_BUDGET_BNB:
                    return {"ok": False, "need_fund": True,
                            "error": f"BNB 余额不足（{bal['native']:.6f} < {TX_GAS_BUDGET_BNB}，USDT 转账 gas），"
    ...
    def open_private(self, peer_agent_id: str, purpose: str = "") -> Session:
        ...
        if not resp.get("ok"):
            raise HubError(f"peer rejected private chat: {resp.get('error', 'unknown')}")
        ...
        if not rsig or _rec(rsig, f"{session_id}:{self.agent_id}".encode("utf-8")) != peer_agent_id.lower():
            raise HubError(f"peer response signature invalid, possible MITM ({peer_agent_id[:12]}…)")
    ...
    def send_private(self, session: Session, text: str) -> dict:
        ...
        🔒 security boundary: 发送前对内容做脱敏（自身凭据恒拦截)，防止被诱导后外泄。
        """
        ...
        if guard.blocked:
            raise HubError(f"security boundary: {guard.reason}")
        if guard.hits:
            print(f"[send] ⚠ message contains sensitive patterns, redacted: {sorted({t for t, _ in guard.hits})}")
    ...
    def open_group(self, member_ids: list[str], topic: str = "") -> dict:
        ...
        if self.local_server is None:
            raise HubError("group owner needs local_server as group service")
        ...
        for member_id in member_ids:
            ...
            if not resp.get("ok"):
                raise HubError(f"member {member_id} rejected group join: {resp.get('error', 'unknown')}")
            rsig = resp.get("responder_signature", "")
            if not rsig or _rec(rsig, f"{group_id}:{self.agent_id}".encode("utf-8")) != member_id.lower():
                raise HubError(f"member {member_id[:12]}… response signature invalid, possible MITM")
    ...
    def send_group(self, group: dict, text: str) -> list[tuple[str, dict]]:
        ...
        if self.local_server is None:
            raise HubError("local_server required")
        ...
        if session is None:
            raise HubError(f"this agent is not in group {group_id}")
